...
Current fork behavior also includes managed Codex identity/request diagnostics, input/cache-based context pressure, recoverable tool-result truncation audits, correction-aware summaries, boolean extension flags that preserve prompts, inherited shell precedence, and graceful timeout cleanup. Truncation audits are file-only on success, keeping console output clean; audit-write failures still abort summarization.
...
Current status: upstream now owns split-turn summary retention and reserve-derived output budgets; the fork no longer carries independent summary-budget settings.
...
Current status: the historical cooldown patches are no longer retained; cancellation and continuation use upstream behavior.
